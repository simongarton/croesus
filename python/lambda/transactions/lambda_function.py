...
def add_transaction(exchange, symbol, date, quantity, price):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO transaction (date, exchange, symbol, quantity, price) "
        "VALUES (%s, %s, %s, %s, %s);",
        [date, exchange, symbol, quantity, price],
    )
...
def get_transactions(exchange, symbol):
    ...
    symbol_transactions = [t for t in exchange_transactions if t[3] == symbol]
